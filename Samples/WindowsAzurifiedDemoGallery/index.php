...
// Controls connections to cloud or local storage
define("PRODUCTION_SITE", false);
define(
    "AZURE_STORAGE_KEY", 
    "bNpZ3C0XHzswv+2W0irgdl/pbFXqc0T80THhwu4sxJ+ion16VrMMrrw/N9z9JhizPF6KK3dlF23yowV9/bhZcQ=="
);
define("AZURE_SERVICE", "pslstorage");
//const ROLE_ID = " ".$_SERVER['RoleDeploymentID'] 
//. '/' . $_SERVER['RoleName'] . '/' . $_SERVER['RoleInstanceID'];
// How many seconds between times dumping performance metrics to table storage
define("PERF_IN_SEC", 30); 

//Mapp Azure Table Storage Handler for Sessions
$table = new Microsoft_WindowsAzure_Storage_Table(
    'table.core.windows.net', 
    AZURE_SERVICE, 
    AZURE_STORAGE_KEY
);
$table->setProxy(true, 'itgproxy.redmond.corp.microsoft.com', 80);
//$table->blob->setProxy(true, 'itgproxy.redmond.corp.microsoft.com', 80);
$sessionHandler = new Microsoft_WindowsAzure_SessionHandler($table, 'sessionstable');
$sessionHandler->register();

session_start(); //Azure Based session
...
if (PRODUCTION_SITE) {
    ...
    $blob->setProxy(true, 'itgproxy.redmond.corp.microsoft.com', 80);
...
$images  = $demoGallery->setupGallery($blob);
